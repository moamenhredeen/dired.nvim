...
higher. The plugin uses a small native helper binary; the `build` step below
downloads a prebuilt binary for your platform and falls back to compiling it
with a [Rust toolchain](https://rustup.rs) (`cargo`) if no prebuilt binary is
available.
